...
spring_cutoff = 3  #ATM21 goes through this month 
fall_cutoff = 11   #ATM22 goes up to this month (not including)
day_cutoff = 15 # day of month for dividing ATM21/ATM22
...
class ListGen(object):
  ...
  def get_data_category(self, query):
    #parses query output for tel_cut_mask
    cat = query.split('\t')[1]
    if cat == 'reducedhv' or cat == 'filter' or cat == 'moonfilter' or cat =='science':
      return '_'+cat
    else:
      return '_other'
  ...
  def get_atm(self, query):
    ...
    if month < spring_cutoff or month > fall_cutoff:
      return '_ATM21'
    elif month == spring_cutoff and day <= day_cutoff or month == fall_cutoff and day >= day_cutoff:
      return '_ATM21'
    else:
      return '_ATM22'
...
def main():
  ...
  for run in lines:
    ...
    execCMD_ver = "select MONTH(data_start_time),DAY(data_start_time),DATEDIFF(data_start_time,'%s'),DATEDIFF(data_start_time,'%s'),config_mask from tblRun_Info where run_id='%s'" %(NA_date,UA_date,runID)
    ...
    data_cat = runsobj.get_data_category(q_offline) #data category: science/filter/rhv/etc.
